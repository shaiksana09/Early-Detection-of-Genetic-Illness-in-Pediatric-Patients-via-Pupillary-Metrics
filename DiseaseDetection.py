...
main = tkinter.Tk()
main.title("Early Detection of Genetic Illness in Pediatric Patients via Pupillary Metrics")
...
def runLSTM():
    ...
    cm = confusion_matrix(temp, prediction_data)
    sensitivity = cm[0,0]/(cm[0,0]+cm[0,1])
    text.insert(END,'LSTM Algorithm Sensitivity : '+str(sensitivity)+"\n")
    specificity = cm[1,1]/(cm[1,0]+cm[1,1])
    text.insert(END,'LSTM Algorithm Specificity : '+str(specificity)+"\n")
...
def runBILSTM():
    ...
    cm = confusion_matrix(temp, prediction_data)
    sensitivity = cm[0,0]/(cm[0,0]+cm[0,1])
    text.insert(END,'BI-LSTM Algorithm Sensitivity : '+str(sensitivity)+"\n")
    specificity = cm[1,1]/(cm[1,0]+cm[1,1])
    text.insert(END,'BI-LSTM Algorithm Specificity : '+str(specificity)+"\n")
...
def graph():
    height = [right_svm_acc,left_svm_acc,ensemble_acc,lstm_acc,bilstm_acc]
    bars = ('Right Pupil SVM Acc','Left Pupil SVM Acc','Ensemble OR (L & R Pupil) Acc','LSTM Acc','BI-LSTM Acc')
...
font = ('times', 16, 'bold')
title = Label(main, text='Early Detection of Genetic Illness in Pediatric Patients via Pupillary Metrics')
title.config(bg='#E6E6FA', fg='black')  
...
predictButton.place(x=850,y=600)
